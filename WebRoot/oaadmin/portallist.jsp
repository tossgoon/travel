...
<body>
			...
			<div  style="padding-top:20px;">
					...
					<div style="margin:0 auto;margin-top:20px;text-align:left;height:400px;">
							<div class="menu1box">
								<ul id="menu1">
									<li <c:if test="${ptype=='0'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=0">简介</a></li>
									<li <c:if test="${ptype=='1'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=1">工作动态</a></li>
									<li <c:if test="${ptype=='2'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=2">政策法规</a></li>
									<li <c:if test="${ptype=='3'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=3">政务公开</a></li>
									<li <c:if test="${ptype=='13'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=13">动植物防护</a></li>
									<li <c:if test="${ptype=='8'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=8">保护区防火</a></li>
									<li <c:if test="${ptype=='9'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=9">保护区防虫</a></li>
									<li <c:if test="${ptype=='5'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=5">科普-动物</a></li>
									<li <c:if test="${ptype=='10'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=10">科普-植物</a></li>
									<li <c:if test="${ptype=='6'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=6">联系我们</a></li>
									<li <c:if test="${ptype=='7'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=7">图片-动物</a></li>
									<li <c:if test="${ptype=='11'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=11">图片-植物</a></li>
									<li <c:if test="${ptype=='12'}"> class= "mhover" </c:if>><a
										href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=12">图片-景观</a></li>
								</ul>
							</div>
									...
									<c:forEach var="portal" items="${portals}">
										<tr align="center" height="27px">
											...
											<td align="center" width="15%"><a
												...
												onclick="window.location.href='<%=contextPath%>/portal/query.action?id=${portal.id}'">编辑</a>&nbsp;&nbsp;
					 ...
					 <div style="padding-bottom:5px;">
						<a id="firstpage"	href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=1&type=${ptype}">第一页</a>
						<a id="lastpage"	href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=${page.currentPage-1 }&type=${ptype}">上一页</a>
						<a id="nextpage"	href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=${page.currentPage+1 }&type=${ptype}">下一页</a>
						<a id="endpage"		href="<%=contextPath%>/portal/querypage.action?pagesize=10&pagenum=${page.totalPage }&type=${ptype}">最后一页</a>
						<label id="pagecount"> ${page.currentPage }/${page.totalPage }</label>
						<a class="btn btn-success" style="margin-left:50px;" target="_blanck" href="<%=contextPath%>/portal/editor.jsp?ptype=${ptype}">新增数据</a>
					</div>
	...
	<script type="text/javascript">
		...
		function DeletePortal() {
			$.ajax({
				url : contextPath+'/portal/delete.action?id=' + delPortalid,
				type : 'POST',
				// 提交数据给Action传入数据
