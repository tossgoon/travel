 ...
 * BD-09：百度坐标偏移标准，Baidu Map使用 WGS-84 to GCJ-02 GPS.gcj_encrypt(); GCJ-02 to
 * WGS-84 粗略 GPS.gcj_decrypt(); GCJ-02 to WGS-84 精确(二分极限法) var threshold =
 * 0.000000001;
 * 目前设置的是精确到小数点后9位，这个值越小，越精确，但是javascript中，浮点运算本身就不太精确，九位在GPS里也偏差不大了
 * GSP.gcj_decrypt_exact(); GCJ-02 to BD-09 GPS.bd_encrypt(); BD-09 to GCJ-02
 * GPS.bd_decrypt(); 求距离 GPS.distance();
 */
var GPS = {
	...
	mercator_encrypt : function(wgsLat, wgsLon) {
		...
		var y = Math.log(Math.tan((90. + wgsLat) * this.PI / 360.))
				/ (this.PI / 180.);
		y = y * 20037508.34 / 180.;
		return {
			'lat' : y,
	...
	distance : function(latA, lonA, latB, lonB) {
		var earthR = 6371000.;
		var x = Math.cos(latA * this.PI / 180.)
				* Math.cos(latB * this.PI / 180.)
				* Math.cos((lonA - lonB) * this.PI / 180);
		var y = Math.sin(latA * this.PI / 180.)
