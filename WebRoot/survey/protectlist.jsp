...
<%@ include file="/oaadmin/oahead.jsp"%>
	<div class="toptool">
		<span>当前位置：OA系统&gt;&gt;保护区巡护记录数据查询 </span>
		...
		<div class="maincontent">
			<div class="leftpanel">
				<ul>
					...
					<li  class="activeli"><a href="javascript:void(0)">保护区巡护查询	</a></li>
			...
			<div class="rightpanel">
				<div class="container"
							...
							<div class="panel panel-primary" style="width:100%;">
								...
								<div class="panel-body container" style="width:100%;">
									...
									<div class="row">
										...
										<div class="col-md-4">
											<s:textfield class="form-control input-sm" id="dongwuming"
												value="%{dongwuming}">
											</s:textfield>
										</div>
									</div>
													...
													<tbody id="tbweather">
														<c:forEach var="a" items="${protectlist}">
															...
															<td>${a.weidu}</td> --%>
																<td>
																<c:if test="${usertype==1}">
																<a	href="<%=contextPath%>/survey/queryprotect.action?id=${a.id}"
					...
					<div class="row">
						<div class="col-md-12">
							<a id="firstpage"
								href="<%=contextPath%>/survey/queryprotectpage.action?pagesize=10&pagenum=1">第一页</a>
							<a id="lastpage"
								href="<%=contextPath%>/survey/queryprotectpage.action?pagesize=10&pagenum=${page.currentPage-1 }">上一页</a>
							<a id="nextpage"
								href="<%=contextPath%>/survey/queryprotectpage.action?pagesize=10&pagenum=${page.currentPage+1 }">下一页</a>
							<a id="endpage"
								href="<%=contextPath%>/survey/queryprotectpage.action?pagesize=10&pagenum=${page.totalPage }">最后一页</a>
    ...
    <script type="text/javascript">
		...
		function DoSearch(){
			...
			var p1=$("#baohuzhan").val();//
			var p2=$("#dongwuming").val();//
			var searchstr="p1="+p1+"&p2="+p2;
			
			if(begindate==""&&enddate==""){
				window.location.href ="<%=contextPath%>/survey/queryprotectpage.action?"+searchstr; 
			}
			else if(begindate==""){
				window.location.href ="<%=contextPath%>/survey/queryprotectpage.action?enddate="+enddate+"&"+searchstr; 
			}
			else if(enddate==""){
				window.location.href ="<%=contextPath%>/survey/queryprotectpage.action?begindate="+begindate+"&"+searchstr; 
			}
			else{
				window.location.href ="<%=contextPath%>/survey/queryprotectpage.action?begindate="+begindate+"&enddate="+enddate+"&"+searchstr; 
			}
		}
		function LookGIS(){
			//导出数据
			...
			var searchstr="p1="+p1+"&p2="+p2;
			if(begindate==""&&enddate==""){
				document.getElementById("lookgis").href="<%=contextPath%>/survey/showprotectlist.action?"+searchstr;
			}
			else if(begindate==""){
				document.getElementById("lookgis").href="<%=contextPath%>/survey/showprotectlist.action?enddate="+enddate+"&"+searchstr;
			}
			else if(enddate==""){
				document.getElementById("lookgis").href="<%=contextPath%>/survey/showprotectlist.action?begindate="+begindate+"&"+searchstr;
			}
			else{
				document.getElementById("lookgis").href="<%=contextPath%>/survey/showprotectlist.action?begindate="+begindate+"&enddate="+enddate+"&"+searchstr;
			}
		}
		function DoExport(){
			//导出数据
			...
			var searchstr="p1="+p1+"&p2="+p2;
			if(begindate==""&&enddate==""){
				document.getElementById("export").href="<%=contextPath%>/survey/exportprotectlist.action?"+searchstr;
			}
			else if(begindate==""){
				document.getElementById("export").href="<%=contextPath%>/survey/exportprotectlist.action?enddate="+enddate+"&"+searchstr;
			}
			else if(enddate==""){
				document.getElementById("export").href="<%=contextPath%>/survey/exportprotectlist.action?begindate="+begindate+"&"+searchstr;
			}
			else{
				document.getElementById("export").href="<%=contextPath%>/survey/exportprotectlist.action?begindate="+begindate+"&enddate="+enddate+"&"+searchstr;
			}
		}
	</script>
